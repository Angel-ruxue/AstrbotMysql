...
from astrbot.core.commands.commands import load_all_commands
from astrbot.core.commands.chatRequest import ai_command
...
class Main(star.Star):
    ...
    # 对接星火大模型
    @filter.command("AI")
    async def handle_ai_command(self, event):
        text = getattr(event, "message_str", "")
        logger.info(f"尝试获取的消息文本 (message_str): '{text}'")

        # 去掉开头的 'AI' 符号和前后空白字符
        user_input = text.lstrip("AI").strip()

        # 向星火大模型发送请求
        reply = ai_command(user_input)

        # 如果 reply 是空的，直接 return
        if not reply or not reply.strip():
            logger.info("AI 回复为空，跳过设置结果")
            return
